...
export default function PaintableSurface({
  ...
  const scorePixels = useMemo(() => new Uint8Array(REDUCE_SIZE * REDUCE_SIZE * 4), [])
  ...
  const splat = useCallback((center, radius, axis, stretch) => {
    ...
    if (center.distanceToSquared(bounds.current.center) > (bounds.current.radius + reach) ** 2) {
      return false
    }
  ...
  const shedFrom = useCallback((uv, radius, direction) => {
    pendingSheds.current.push({
      u: uv.x, v: uv.y, radius, dx: direction.x, dy: direction.y, dz: direction.z,
    })
  }, [])
  ...
  const shedFromCell = useCallback((cell, shot) => {
    ...
    let toEdge = Infinity
    if (directionU > 1e-6) toEdge = Math.min(toEdge, (cell.faceMax.x - shot.u) / directionU)
    else if (directionU < -1e-6) toEdge = Math.min(toEdge, (cell.faceMin.x - shot.u) / directionU)
    if (directionV > 1e-6) toEdge = Math.min(toEdge, (cell.faceMax.y - shot.v) / directionV)
    else if (directionV < -1e-6) toEdge = Math.min(toEdge, (cell.faceMin.y - shot.v) / directionV)
    ...
    const radiusUv = shot.radius * Math.hypot(
    ...
    cellPointToWorld(cell, shot.u + directionU * toEdge, shot.v + directionV * toEdge, lipPoint)
  ...
  useEffect(() => {
    ...
    bounds.current.copy(geometry.boundingSphere).applyMatrix4(mesh.current.matrixWorld)
  ...
  }, [geometry, gl, gridX, gridY, id, maskSize, registerSurface, shedFrom, splat, targets])
  ...
  useFrame((state, rawDelta) => {
    ...
    while (pendingSheds.current.length > 0) {
      ...
      const column = THREE.MathUtils.clamp(Math.floor(shot.u * gridX), 0, gridX - 1)
      const row = THREE.MathUtils.clamp(Math.floor(shot.v * gridY), 0, gridY - 1)
    ...
    if (!scoreDirty.current || now - lastScore.current < SCORING.interval) return
    lastScore.current = now
    scoreDirty.current = false

    const uniforms = passes.reduceMaterial.uniforms
    uniforms.uMask.value = targets.read.texture
    uniforms.uStep.value.set(1 / (REDUCE_SIZE * REDUCE_TAPS), 1 / (REDUCE_SIZE * REDUCE_TAPS))
    passes.render(gl, passes.reduceMaterial, targets.reduce, false)
    gl.readRenderTargetPixels(targets.reduce, 0, 0, REDUCE_SIZE, REDUCE_SIZE, scorePixels)

    let total = 0
    for (let pixel = 0; pixel < scorePixels.length; pixel += 4) total += scorePixels[pixel]
    onCoverage(id, total / (255 * REDUCE_SIZE * REDUCE_SIZE), weight)
  })

  return <mesh ref={mesh} geometry={geometry} material={material} position={position} rotation={rotation} />
}
