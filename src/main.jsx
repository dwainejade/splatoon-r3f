import { StrictMode, forwardRef, useCallback, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react'
...
import { ARENA, DEFAULT_WEAPON, INK, PAINT, PLAYER, PROJECTILE_CAPACITY, RUN } from './settings.js'
import './styles.css'
...
function FirstPersonMovement() {
  ...
  useFrame((_, delta) => {
    ...
    if (movement.lengthSq() > 0) {
      ...
      movement.normalize().multiplyScalar(PLAYER.moveSpeed * Math.min(delta, 1 / 30))
      camera.position.add(movement)
      camera.position.x = THREE.MathUtils.clamp(camera.position.x, -PLAYER.arenaLimit, PLAYER.arenaLimit)
      camera.position.z = THREE.MathUtils.clamp(camera.position.z, -PLAYER.arenaLimit, PLAYER.arenaLimit)
...
function Arena({ onCoverage, onSpray, registerSurface }) {
  const floorGeometry = useMemo(() => new THREE.PlaneGeometry(ARENA.floorSize, ARENA.floorSize), [])
  const blockGeometries = useMemo(
    () => ARENA.blocks.map(([, , , width, height, depth]) => createAtlasBoxGeometry(width, height, depth)),
    [],
  )
  ...
  return <>
    ...
    <directionalLight position={ARENA.lightDirection} intensity={ARENA.sunIntensity} />
    <PaintableSurface id="floor" geometry={floorGeometry} gridX={1} gridY={1} worldSpan={ARENA.floorSize}
      position={[0, 0, 0]} rotation={[-Math.PI / 2, 0, 0]} baseColor={ARENA.floorColor} inkColor={PAINT.color}
      lightDirection={ARENA.lightDirection} weight={ARENA.floorSize * ARENA.floorSize}
      onCoverage={onCoverage} onSpray={onSpray} registerSurface={registerSurface} />
    {ARENA.blocks.map(([x, y, z, width, height, depth], index) => (
      <PaintableSurface key={index} id={`block-${index}`} geometry={blockGeometries[index]} gridX={3} gridY={2}
        worldSpan={3 * Math.max(width, height, depth)} position={[x, y, z]}
        baseColor={ARENA.blockColors[index % ARENA.blockColors.length]} inkColor={PAINT.color} lightDirection={ARENA.lightDirection}
...
function World({ runId, paintEnabled, ink, onCoverage, surfaces, onInkUse, registerSurface }) {
  const projectiles = useRef()
  const droplets = useRef()
  const sync = useCallback((shots) => projectiles.current?.sync(shots), [])
  const spray = useCallback(
    (position, radius, delay, velocity) => droplets.current?.spawn(position, radius, delay, velocity),
    [],
  )
  ...
  const paint = useCallback((center, radius, axis, stretch) => {
    for (const surface of surfaces.current.values()) surface.splat(center, radius, axis, stretch)
  }, [surfaces])
  return <Canvas camera={{ position: PLAYER.spawn, fov: PLAYER.fieldOfView }}>
    ...
    <InkWeapon weapon={DEFAULT_WEAPON} paintEnabled={paintEnabled} ink={ink}
      surfaces={surfaces} paint={paint} onInkUse={onInkUse} onSync={sync} />
    <Arena key={runId} onCoverage={onCoverage} onSpray={spray} registerSurface={registerSurface} />
  </Canvas>
}
...
function App() {
  ...
  const consumeInk = useCallback((amount = 1.5) => setInk((value) => Math.max(0, value - amount)), [])
  ...
  const reportCoverage = useCallback((id, coverage, weight) => {
    ...
    const coveredWeight = measured.reduce((total, surface) => total + surface.coverage * surface.weight, 0)
  ...
  return <main>
    <World runId={runId} paintEnabled={paintEnabled} ink={ink} onCoverage={reportCoverage}
      surfaces={surfaces} onInkUse={consumeInk} registerSurface={registerSurface} />
    <div className="hud">
      <div className="brand">INK RUSH <span>prototype</span></div>
      <div className="timer">{minutes}:{seconds}</div>
      <div className="score" aria-live="off"><span>Painted</span><strong>{painted.toFixed(1)}%</strong></div>
      <div className="ink"><span>INK</span><div><i style={{ width: `${(ink / INK.capacity) * 100}%` }} /></div></div>
    </div>
    <div className="crosshair" aria-hidden="true">+</div>
    {!running && <section className="start-card">
      {remaining === 0
        ? <p>Run complete — you covered <strong>{painted.toFixed(1)}%</strong>.</p>
        : <p>A first-person paint time attack.</p>}
      <button onClick={startRun}>{remaining === 0 ? 'Run it again' : 'Start 3-minute run'}</button>
      <small>Click the arena to look around · WASD to move · Hold left click to spray · Hold right click for a burst</small>
    </section>}
  </main>
}
