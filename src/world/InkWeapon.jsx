...
export default function InkWeapon({
  ...
  useEffect(() => {
    ...
    const up = (event) => {
      ...
      if (event.button === 2 && chargeStarted.current !== null) {
        ...
        if (chargeTime > 0.08)
          fireNow.current(Math.min(1, chargeTime * 0.8), weapon.chargeInkCost);
      }
    };
  ...
  useFrame((state, rawDelta) => {
    ...
    if (
      held.current &&
      paintEnabled &&
      now - lastShot.current >= weapon.fireInterval
    ) {
    ...
    for (const shot of projectiles) {
      ...
      shot.visualRadius =
        ...
        Math.min(1, shot.distance / weapon.emergenceDistance);
      ...
      if (travelled > 1e-6 && meshes.length > 0) {
        ...
        if (hit) {
          ...
          const radius = Math.min(
            weapon.maxRadius,
            shot.radius + weapon.radiusPerMetre * shot.distance,
          );
      ...
      if (shot.age >= weapon.lifetime || shot.position.y < -4)
        shot.alive = false;
    }
